...
if [ "${{1:-}}" = "tool" ] && [ "${{2:-}}" = "install" ]; then
    ...
    cp "$FAKE_FIXTURES/fcc-command.sh" "$FAKE_TOOL_BIN/pcc-server"
    cp "$FAKE_FIXTURES/fcc-command.sh" "$FAKE_TOOL_BIN/pcc-desktop"
    # Under fcc-missing, omit pcc-claude so install.sh fails its post-install
    # verification ("did not create ...") before running `pcc-server --version`.
    if [ "$FAIL_STEP" != "fcc-missing" ]; then
        cp "$FAKE_FIXTURES/fcc-command.sh" "$FAKE_TOOL_BIN/pcc-claude"
    fi
...
if [ "$name" = "pcc-server" ] && [ "${1:-}" = "--version" ]; then
...
def test_install_sh_fresh_install_is_verified(posix_harness: PosixHarness) -> None:
    ...
    assert any(
        call.startswith(
            ...
            "https://github.com/King-Jboy/kingjboy-claude-code/archive/refs/heads/main.zip"
        )
        for call in calls
    )
...
def test_install_sh_creates_native_macos_app_and_desktop_link(
    ...
    launcher = app / "Contents" / "MacOS" / "pcc-desktop"
    ...
    expected_command = str(tool_bin / "pcc-desktop").replace("'", "'\\''")
...
def test_install_sh_stops_without_success_on_each_failure(
    ...
    forbidden = {
        ...
        "fcc-missing": "pcc-server:--version",
    }.get(failure)
...
def test_install_sh_rechecks_for_fcc_process_before_tool_replacement(
    ...
    posix_harness.env["FCC_RUNNING_COMMAND"] = "pcc-server"
    posix_harness.env["FCC_RUNNING_PHASE"] = "late"

    result = posix_harness.run()

    assert result.returncode != 0
    assert "pcc-server (PID 4242)" in result.stderr
...
def test_install_sh_ignores_similarly_named_process(
    ...
    posix_harness.env["FCC_RUNNING_COMMAND"] = "pcc-server-helper"
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("pcc-server", "/usr/bin/python3 /home/user/.local/bin/pcc-server"),
    ),
)
...
copy /y "%FAKE_FIXTURES%\fcc-command.cmd" "%FAKE_TOOL_BIN%\pcc-server.cmd" >nul
copy /y "%FAKE_FIXTURES%\fcc-command.cmd" "%FAKE_TOOL_BIN%\pcc-desktop.cmd" >nul
copy /y "%FAKE_FIXTURES%\fcc-command.cmd" "%FAKE_TOOL_BIN%\pcc-claude.cmd" >nul
exit /b 0
:update_shell
...
if "%FCC_NAME%"=="pcc-server" if "%1"=="--version" echo free-claude-code 3.5.18
exit /b 0
""",
        encoding="utf-8",
    )
...
def test_install_ps1_rechecks_for_fcc_process_before_tool_replacement(
    ...
    powershell_harness.env["FCC_RUNNING_COMMAND"] = "pcc-server"
...
def test_install_ps1_ignores_similarly_named_process(
    ...
    powershell_harness.env["FCC_RUNNING_COMMAND"] = "pcc-server-helper"
