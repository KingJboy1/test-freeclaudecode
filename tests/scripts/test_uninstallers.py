...
if [ "${1:-}" = "tool" ] && [ "${2:-}" = "uninstall" ]; then
    ...
    for name in pcc-server pcc-claude free-claude-code; do
...
def test_readme_uninstall_uses_raw_urls_and_verification_contract() -> None:
    ...
    assert (
        ...
        "King-Jboy/kingjboy-claude-code/main/scripts/uninstall.sh | sh"
    ) in text
    assert (
        '& ([scriptblock]::Create((irm "https://raw.githubusercontent.com/'
        'King-Jboy/kingjboy-claude-code/main/scripts/uninstall.ps1")))'
    ) in text
