...
install_macos_desktop_app() {
    ...
    executable_path="$executable_dir/pcc-desktop"
...
PLIST
    desktop_command=$(shell_quote "$tool_bin/pcc-desktop")
    {
        printf '%s\n' '#!/bin/sh'
        printf 'exec %s\n' "$desktop_command"
    } > "$executable_path"
