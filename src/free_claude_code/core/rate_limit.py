...
class StrictSlidingWindowLimiter:
    ...
    def count_in_window(self) -> int:
        """Return acquisitions currently inside the sliding window.

        Synchronous: prunes expired timestamps and returns the live count without
        awaiting. Safe for key-rotation predictive pacing because asyncio is
        single-threaded and no await occurs mid-operation.
        """
        now = time.monotonic()
        cutoff = now - self._rate_window
        while self._times and self._times[0] <= cutoff:
    ...
    def record_dispatch(self) -> None:
        """Record one dispatch timestamp without awaiting capacity.

        Used by per-key rotation limiters to track actual usage for predictive
        pacing. The hard RPM gate stays with the provider admission controller;
        this only feeds the synchronous near-ceiling skip check.
        """
        self._times.append(time.monotonic())
